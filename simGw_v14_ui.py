...
class SimGwV2App:
    def __init__(self, root: tk.Tk) -> None:
        ...
        # Demo state: keep last overall and last waveform separately
        self._demo_last_overall_rx: str = ""
        self._demo_last_wave_rx: str = ""
        self._demo_last_export_info: Optional[dict] = None
        self._demo_mpl_ready: bool = False
        self._demo_plot_canvas = None
        self._demo_plot_fig = None
        self._demo_plot_ax = None
    ...
    def _build_ui_demo(self, parent: tk.Frame) -> None:
        ...
        btns.pack(anchor="e")
        ttk.Button(btns, text="Start", style="Accent.TButton", command=self._on_start).pack(side=tk.LEFT, padx=(0, 6))
        ttk.Button(btns, text="Stop", command=self._stop_auto).pack(side=tk.LEFT, padx=(0, 6))
        ttk.Button(btns, text="Plot Latest", command=self._plot_latest_waveform).pack(side=tk.LEFT)

        # Status / summary KPIs
        meta = tk.Frame(panel, bg=self.colors["panel"], highlightbackground=self.colors["border"], highlightthickness=1)
        meta.pack(fill=tk.X, pady=(0, 12))
        meta_in = tk.Frame(meta, bg=self.colors["panel"])
        meta_in.pack(fill=tk.X, padx=14, pady=12)

        def _row(title: str, var: tk.StringVar) -> None:
            r = tk.Frame(meta_in, bg=self.colors["panel"])
            r.pack(fill=tk.X, pady=2)
            tk.Label(r, text=title, bg=self.colors["panel"], fg=self.colors["muted"],
                     font=("Segoe UI", 10, "bold")).pack(side=tk.LEFT)
            tk.Label(r, textvariable=var, bg=self.colors["panel"], fg=self.colors["text"]).pack(side=tk.LEFT, padx=(8, 0))

        _row("Status", self.demo_status_var)
        _row("Device", self.demo_device_var)
        _row("Overall", self.demo_overall_var)
        _row("Waveform", self.demo_waveform_var)

        # Summary + waveform plot (embedded)
        sum_box = tk.Frame(panel, bg=self.colors["panel"], highlightbackground=self.colors["border"], highlightthickness=1)
        sum_box.pack(fill=tk.BOTH, expand=True)

        sum_in = tk.Frame(sum_box, bg=self.colors["panel"])
        sum_in.pack(fill=tk.BOTH, expand=True, padx=14, pady=12)

        tk.Label(sum_in, text="Overall & Waveform", bg=self.colors["panel"], fg=self.colors["text"],
                 font=("Segoe UI", 11, "bold")).pack(anchor="w")

        body = tk.Frame(sum_in, bg=self.colors["panel"])
        ...
        self.demo_summary = tk.Text(left, height=16, wrap=tk.WORD, bg=self.colors["panel"],
                                    fg=self.colors["text"], bd=0, highlightthickness=0)
        self.demo_summary.pack(fill=tk.BOTH, expand=True)
        ...
        # Embedded plot
        self.demo_plot_status = tk.StringVar(value="Waveform plot: waiting for data…")
        tk.Label(right, textvariable=self.demo_plot_status, bg=self.colors["panel"], fg=self.colors["muted"]).pack(anchor="w")

        plot_host = tk.Frame(right, bg=self.colors["panel"])
        plot_host.pack(fill=tk.BOTH, expand=True, pady=(6, 0))

        try:
            from matplotlib.backends.backend_tkagg import FigureCanvasTkAgg
            from matplotlib.figure import Figure

            self._demo_plot_fig = Figure(figsize=(4.0, 2.6), dpi=100)
            self._demo_plot_ax = self._demo_plot_fig.add_subplot(111)
            self._demo_plot_ax.set_title("Waveform")
            self._demo_plot_ax.set_xlabel("Sample")
            self._demo_plot_ax.set_ylabel("Amplitude (int16)")
            self._demo_plot_canvas = FigureCanvasTkAgg(self._demo_plot_fig, master=plot_host)
            self._demo_plot_canvas.get_tk_widget().pack(fill=tk.BOTH, expand=True)
            self._demo_mpl_ready = True
        except Exception as mpl_exc:
            self._demo_mpl_ready = False
            tk.Label(plot_host, text=f"Matplotlib not available: {mpl_exc}", bg=self.colors["panel"], fg=self.colors["muted"]).pack(anchor="w")
    ...
    def _demo_set_kpis_from_rx_text(self, rx_text: str, export_info: Optional[dict]) -> None:
        ...
        # Overall KPI: detect overall data_upload (non-wave)
        if msg_type == "data_upload" and "VIBRATION_ACC_WAVE" not in rx_text:
            pairs = rx_text.count("data_pair")
            # heuristic: 'data_pair' also appears in nested text; use 'data_pair {' pattern if present
            pairs2 = len(re.findall(r"\bdata_pair\s*\{", rx_text))
            pairs = pairs2 if pairs2 > 0 else pairs
            if pairs > 0:
                self.demo_overall_var.set(f"OK ({pairs} pairs)")

    def _demo_render_summary(self, rx_text: str) -> None:
        """Render a readable key/value summary in the Demo tab from the worker rx_text."""
        if self.demo_summary is None:
            return
        text = rx_text or ""
        # Extract TYPE
        m = re.search(r"^TYPE:\s*([^\n]+)", text.strip(), flags=re.MULTILINE)
        msg_type = m.group(1).strip() if m else ""

        # Remove HEX line and any EXPORT section from the text
        lines = text.splitlines()
        clean = []
        for ln in lines:
            if ln.startswith("HEX:"):
                continue
            if ln.startswith("EXPORT:") or ln.startswith("EXPORT "):
                continue
            clean.append(ln)
        clean_text = "\n".join(clean)

        # Compute waveform/overall specifics for nicer rendering
        is_wave = "VIBRATION_ACC_WAVE" in clean_text
        total_block = None
        m2 = re.search(r"\btotal_block:\s*(\d+)", clean_text)
        if m2:
            try:
        ...
        pairs = len(re.findall(r"\bdata_pair\s*\{", clean_text))
        ...
        # Type
        self.demo_summary.insert(tk.END, "Type: ", ("k",))
        self.demo_summary.insert(tk.END, f"{msg_type or '(unknown)'}\n", ("v",))

        if msg_type == "data_upload":
            if is_wave:
                blocks = total_block if total_block is not None else "?"
                pts = (total_block * 64) if isinstance(total_block, int) else "?"
                self.demo_summary.insert(tk.END, "Waveform: ", ("k",))
                self.demo_summary.insert(tk.END, f"{blocks} blocks ({pts} points)\n", ("v",))
                self.demo_summary.insert(tk.END, "Measurement data: ", ("k",))
                self.demo_summary.insert(tk.END, f"{pts} points (int16)\n", ("v",))
            else:
                self.demo_summary.insert(tk.END, "Overall: ", ("k",))
                self.demo_summary.insert(tk.END, f"{pairs} pairs\n", ("v",))
                self.demo_summary.insert(tk.END, "Measurement data: ", ("k",))
                self.demo_summary.insert(tk.END, f"{pairs} pairs\n", ("v",))

        # Optionally show a couple of key header-like fields if present
        for key in ["message_seq_no", "current_block", "config_hash", "timestamp_ms", "time_ms"]:
            mm = re.search(rf"\b{key}:\s*([^\n]+)", clean_text)
            if mm:
                self.demo_summary.insert(tk.END, f"{key}: ", ("k",))
                self.demo_summary.insert(tk.END, mm.group(1).strip() + "\n", ("v",))

        self.demo_summary.configure(state=tk.DISABLED)



    def _demo_ingest_rx(self, rx_text: str, export_info: Optional[dict]) -> None:
        """Classify RX as Overall or Waveform and refresh Demo summary + plot.

        We must keep Overall and Waveform separately because the 'latest RX' often ends as waveform.
        """
        if export_info:
            self._demo_last_export_info = export_info

        # If rx_text is provided, decide where to store it.
        if rx_text:
            # Accept both 'TYPE:' and 'Type:' variants (case-insensitive)
            m = re.search(r"^type:\s*([^\n]+)", rx_text.strip(), flags=re.IGNORECASE | re.MULTILINE)
            msg_type = m.group(1).strip() if m else ""

            is_data_upload = (msg_type.lower() == "data_upload")
            is_wave = False
            if is_data_upload:
                # Heuristics: waveform uploads contain total_block/current_block or measurement_data.data blocks
                if re.search(r"\btotal_block\s*:", rx_text) or re.search(r"\bcurrent_block\s*:", rx_text):
                    is_wave = True
                if "VIBRATION_ACC_WAVE" in rx_text:
                    is_wave = True

            # export_info is the strongest indicator of a waveform capture being completed
            if export_info and is_data_upload:
                is_wave = True

            if is_data_upload and is_wave:
                self._demo_last_wave_rx = rx_text
            elif is_data_upload:
                self._demo_last_overall_rx = rx_text

        # Refresh combined summary and plot
        self._demo_render_combined_summary()
        self._demo_update_plot()

    def _demo_render_combined_summary(self) -> None:
        if self.demo_summary is None:
            return

        overall_items = self._demo_extract_overall_items(self._demo_last_overall_rx)
        wave_meta = self._demo_extract_waveform_meta(self._demo_last_wave_rx, self._demo_last_export_info)
        ...
        if overall_items:
            for k, v in overall_items:
                self.demo_summary.insert(tk.END, f"• {k}: ", ("k",))
                self.demo_summary.insert(tk.END, f"{v}\n", ("v",))
        else:
            self.demo_summary.insert(tk.END, "—\n", ("v",))

        self.demo_summary.insert(tk.END, "\nWaveform\n", ("h",))
        if wave_meta:
            for k, v in wave_meta:
                self.demo_summary.insert(tk.END, f"• {k}: ", ("k",))
                self.demo_summary.insert(tk.END, f"{v}\n", ("v",))
        else:
            self.demo_summary.insert(tk.END, "—\n", ("v",))
    ...
    def _demo_extract_overall_items(self, rx_text: str):
        """Return list of (label, value) from a non-wave data_upload message."""
        if not rx_text:
            return []

        # Strip HEX and EXPORT blocks to avoid noise
        lines = []
        skip_export = False
        for ln in rx_text.splitlines():
            if ln.startswith("HEX:"):
                continue
        ...
        # Find all data_pair { ... } blocks with a small brace parser.
        blocks = []
        i = 0
        while True:
            j = text.find("data_pair", i)
            if j < 0:
                break
            k = text.find("{", j)
            if k < 0:
        ...
        for b in blocks:
            ...
            label = self._demo_friendly_name(nm)

            # Extract scalar value(s)
            val = self._demo_extract_value_from_datapair_block(b)
            if val is None:
                continue
    ...
    def _demo_extract_value_from_datapair_block(self, block: str):
        """
        Extract a human-readable value from a data_pair { ... } text block.

        Reality check: many of our pairs do NOT use the generic oneof "value".
        They may contain a typed sub-message, e.g.:
            data_pair { name: VIBRATION_ACC  vibration_acc { x: ... y: ... z: ... } }

        Strategy:
        1) Look for common scalar "*_value" fields.
        2) Look for "value { ... }" and flatten its scalar fields.
        3) Otherwise, find the first nested field "something { ... }" (excluding data_pair itself)
           and flatten scalar leaf fields inside that region (up to a reasonable depth).
        """
        if not block:
            return None

        # 1) Common scalar fields in oneof 'value'
        scalar_fields = [
            ("float_value", float),
            ("double_value", float),
            ("int32_value", int),
            ...
            ("uint32_value", int),
            ("uint64_value", int),
            ("bool_value", lambda x: x.lower() == "true"),
            ("string_value", str),
        ]
        for key, conv in scalar_fields:
        ...
        def _slice_brace_region(s: str, brace_open_idx: int):
            depth = 0
            end = None
            for p in range(brace_open_idx, len(s)):
                if s[p] == "{":
                    depth += 1
                elif s[p] == "}":
                    depth -= 1
                    if depth == 0:
                        ...
                        break
            return (brace_open_idx, end) if end else (brace_open_idx, None)

        def _flatten_leaf_scalars(msg_region: str):
            """
            Collect scalar-looking 'key: value' leaves from any depth.
            This is intentionally heuristic; it produces nice demo output without schema knowledge.
            """
            out = []
            for line in msg_region.splitlines():
                line = line.strip()
                if not line or line.endswith("{") or line == "}" or line.startswith("#"):
                    continue
                mm = re.match(r"([a-zA-Z0-9_]+)\s*:\s*(.+)", line)
                if not mm:
                    continue
                fk = mm.group(1).strip()
                fv = mm.group(2).strip().strip('"')
                if fk in ("data", "bytes_value"):  # noisy blobs
                    continue
                out.append((self._demo_friendly_name(fk), fv))
        ...
        # 2) Nested message inside value { ... }
        m = re.search(r"\bvalue\s*\{", block)
        if m:
            k = block.find("{", m.start())
            if k >= 0:
                _b, e = _slice_brace_region(block, k)
                if e:
                    val_block = block[k+1:e-1]
                    pairs = _flatten_leaf_scalars(val_block)
                    if not pairs:
                        return "(data)"
                    return "; ".join([f"{k}: {v}" for k, v in pairs])

        # 3) Fallback: find first nested field "<field_name> { ... }" after the 'name:' line.
        # Remove the 'name: ...' line to avoid matching it.
        b2 = "\n".join([ln for ln in block.splitlines() if not re.match(r"\s*name\s*:", ln)])
        m2 = re.search(r"\b([a-zA-Z0-9_]+)\s*\{", b2)
        if not m2:
            return None
        # Find brace start from original block at/after match position
        brace_idx = b2.find("{", m2.start())
        if brace_idx < 0:
            return None
        # Map brace_idx in b2 back to a substring: easiest is to slice from b2
        _b, e = _slice_brace_region(b2, brace_idx)
        if not e:
            return None
        region = b2[brace_idx+1:e-1]
        pairs = _flatten_leaf_scalars(region)
        if not pairs:
            return "(data)"
        return "; ".join([f"{k}: {v}" for k, v in pairs])

    def  _demo_friendly_name(self, name: str) -> str:
        # Enum-ish names: VIBRATION_ACC -> Vibration acc
        n = name.strip()
        n = n.replace("_", " ").strip()
        # Title-case but keep acronyms reasonable
        return n.title()

    def _demo_extract_waveform_meta(self, rx_text: str, export_info: Optional[dict]):
        if not rx_text and not export_info:
            return []
        items = []
        # blocks/points from rx_text if present
        total_block = None
        if rx_text:
            m = re.search(r"\btotal_block\s*:\s*(\d+)", rx_text)
            if m:
                try:
                    total_block = int(m.group(1))
                except Exception:
                    total_block = None
            m2 = re.search(r"\bmessage_seq_no\s*:\s*(\d+)", rx_text)
            if m2:
                items.append(("Message Seq No", m2.group(1)))
        if export_info and export_info.get("count") is not None:
            try:
                total_block = int(export_info.get("count"))
            except Exception:
                pass

        if total_block is not None:
            items.append(("Blocks", str(total_block)))
            items.append(("Points", str(total_block * 64)))
        elif export_info and (export_info.get("raw") or export_info.get("index") or export_info.get("samples")):
            items.append(("Export", "OK"))

        if export_info and export_info.get("raw"):
            items.append(("Raw File", os.path.basename(str(export_info.get("raw")))))
        return items

    def _demo_update_plot(self) -> None:
        if not self._demo_mpl_ready or (self._demo_plot_ax is None) or (self._demo_plot_canvas is None):
            return
        export_info = self._demo_last_export_info or {}
        raw_path = export_info.get("raw") if export_info else None
        if not raw_path or not os.path.exists(raw_path):
            # nothing to plot yet
            self.demo_plot_status.set("Waveform plot: waiting for data…")
            self._demo_plot_ax.clear()
            self._demo_plot_ax.set_title("Waveform")
            self._demo_plot_ax.set_xlabel("Sample")
            self._demo_plot_ax.set_ylabel("Amplitude (int16)")
            self._demo_plot_canvas.draw_idle()
            return

        try:
            y, meta = WaveformExportTools.extract_true_twf_samples_from_raw_export(raw_path)
            self._demo_plot_ax.clear()
            self._demo_plot_ax.plot(y)
            self._demo_plot_ax.set_title(f"Waveform ({meta.get('samples','?')} samples)")
            self._demo_plot_ax.set_xlabel("Sample")
            self._demo_plot_ax.set_ylabel("Amplitude (int16)")
            self.demo_plot_status.set(f"Waveform plot: OK ({meta.get('samples','?')} samples)")
            self._demo_plot_canvas.draw_idle()
        except Exception as exc:
            self.demo_plot_status.set(f"Waveform plot: failed ({type(exc).__name__})")
            self._demo_plot_ax.clear()
            self._demo_plot_ax.set_title("Waveform (error)")
            self._demo_plot_canvas.draw_idle()
            print(f"[DEMO] plot error: {type(exc).__name__}: {exc}")
    ...
    def _apply_tile_update(self, tile_id: int, payload: Dict[str, str]) -> None:
        ...
        # Demo view mirrors the most recent tile for at-a-glance visibility.
        try:
            if self.tiles and (tile_id == max(self.tiles.keys())):
                if "status" in payload:
                    self.demo_status_var.set(payload.get("status", ""))
                if "address" in payload or "name" in payload:
                ...
                export_info = payload.get("export_info") if payload.get("export_info") else None
                if "rx_text" in payload:
                    rx_txt = payload.get("rx_text", "") or ""
                    self._demo_ingest_rx(rx_txt, export_info)
                elif export_info:
                    # Some updates may only carry export_info; still refresh plot/summary.
                    self._demo_ingest_rx("", export_info)
        except Exception as demo_exc:
            print(f"[DEMO] update error: {type(demo_exc).__name__}: {demo_exc}")

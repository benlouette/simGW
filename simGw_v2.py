...
class BleCycleWorker:
	...
	async def _run_cycle(self, tile_id: int, address_prefix: str, mtu: int, scan_timeout: float, rx_timeout: float) -> None:
		...
		try:
			...
			payload = app_message.SerializeToString()
			...
			await client.write_gatt_char(self.uart_rx_uuid, app_message.SerializeToString())

			try:
				await asyncio.wait_for(rx_event.wait(), timeout=rx_timeout)
			except asyncio.TimeoutError:
				self.ui_queue.put(("tile_update", tile_id, {"status": "RX timeout"}))
			else:
				payload = rx_data["value"] or b""
				latest_status = "Received"
				latest_rx_text = self._format_rx_payload(payload)
				try:
					app_message = DeviceAppBulletSensor_pb2.AppMessage()
					app_message.ParseFromString(payload)
					...
					if message_type == "current_version_upload":
						...
						try:
							...
							hash_payload = rx_data["value"] or b""
							try:
								# print("Parsing config hash upload...")
								hash_message = DeviceAppBulletSensor_pb2.AppMessage()
								hash_message.ParseFromString(hash_payload)
								...
								if hash_type == "config_hash_upload":
									...
									for loop_index in range(6):
										...
										latest_rx_text = self._format_rx_payload(hash_payload)
										...
										try:
											...
											data_payload = rx_data["value"] or b""
											try:
												data_message = DeviceAppBulletSensor_pb2.AppMessage()
												data_message.ParseFromString(data_payload)
												...
												if data_type == "data_upload":
													...
													if len(data_pairs) >= 3:
														...
														data_collection_complete = False
													latest_rx_text = self._format_rx_payload(data_payload)
												else:
			...
			self.ui_queue.put(("tile_update", tile_id, {"status": f"Error: {exc}"}))
		finally:

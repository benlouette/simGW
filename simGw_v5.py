        ...
        try:
            if self.tiles and (tile_id == max(self.tiles.keys())):
                ...
                if "rx_text" in payload:
                    ...
                    # Any waveform export present? Plot from export_info without relying on string matching in rx_text.
                    # (In practice, data_upload blocks may not contain the literal token "VIBRATION_ACC_WAVE" in text_format.)
                    if export_info and isinstance(export_info, dict):
                        raw_path = export_info.get("raw")
                        samples_path = export_info.get("samples")
                        # Plot only if we have a new raw export path (avoid replotting every UI update).
                        try:
                            raw_key = str(raw_path) if raw_path else ""
                            if raw_key and (raw_key != str(getattr(self, "_demo_last_plotted_raw", "") or "")) and os.path.exists(raw_key):
                                y, meta = WaveformExportTools.extract_true_twf_samples_from_raw_export(raw_key)
                                if self.demo_plot_canvas is not None and self.demo_plot_fig is not None:
                                    ax = self.demo_plot_fig.axes[0] if self.demo_plot_fig.axes else self.demo_plot_fig.add_subplot(111)
                                    ax.clear()
                                    # Plot index; keep it simple and fast.
                                    ax.plot(list(range(len(y))), y)
                                    ax.set_title(f"Waveform (reconstructed, {meta.get('samples', len(y))} samples)")
                                    ax.set_xlabel("Sample")
                                    ax.set_ylabel("Amplitude (int16)")
                                    ax.grid(True, alpha=0.2)
                                    self.demo_plot_canvas.draw()
                                    if self.demo_plot_label is not None:
                                        self.demo_plot_label.configure(text=f"{meta.get('samples', len(y))} samples")
                                self._demo_last_plotted_raw = raw_key
                                self.demo_waveform_var.set(f"{meta.get('samples', len(y))} samples")
                            elif (not raw_key) and samples_path and os.path.exists(str(samples_path)):
                                # Fallback: plot from samples.csv if provided
                                self._demo_plot_waveform_from_samples(str(samples_path))
                                self.demo_waveform_var.set("Waveform received")
                        except Exception as e:
                            # Surface the error in the UI (otherwise it's impossible to debug)
                            try:
                                if self.demo_plot_label is not None:
                                    self.demo_plot_label.configure(text=f"(plot error: {type(e).__name__})")
                            except Exception:
                                pass

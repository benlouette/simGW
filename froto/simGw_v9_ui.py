...
plt = None  # Lazy-imported in popup plot functions (if used)
...
class SimGwV2App:
    def __init__(self, root: tk.Tk) -> None:
        ...
        # Demo embedded waveform plot
        self.demo_plot_fig = None
        self.demo_plot_canvas = None
        self.demo_plot_widget = None
        self.demo_plot_ax = None
        self.demo_plot_info_var = tk.StringVar(value="No waveform yet")
    ...
    def _demo_update_embedded_waveform(self, export_info: Optional[dict]) -> None:
        """Update the embedded waveform plot in Demo tab from the latest export_info."""
        if self.demo_plot_ax is None or self.demo_plot_canvas is None or self.demo_plot_fig is None:
            return
        if not export_info or not isinstance(export_info, dict):
            return

        # Prefer raw export reconstruction
            ...
            try:
                if samples_path and os.path.exists(samples_path):
                    with open(samples_path, "r", newline="", encoding="utf-8") as f:
                        ...
                        rows = list(r)
                    if rows:
                        # Heuristic: prefer column named 'value' else last numeric column
                        ...
                        for h in cols:
                            ...
                            if ok:
                                cand.append(h)
                        col = "value" if "value" in cand else (cand[-1] if cand else None)
                        if col:
                            y = [float(row.get(col, 0) or 0) for row in rows]
                            src = f"samples.csv ({len(y)} pts)"
            except Exception:
                y = None

        if y is None or len(y) == 0:
            self.demo_plot_info_var.set("No samples to plot")
            return

        try:
            self.demo_plot_ax.clear()
            self.demo_plot_ax.plot(list(range(len(y))), y)
            self.demo_plot_ax.set_title("Latest waveform")
            self.demo_plot_ax.set_xlabel("Sample")
            self.demo_plot_ax.set_ylabel("Value (int16)")
            self.demo_plot_ax.grid(True, alpha=0.2)
            self.demo_plot_canvas.draw_idle()
            self.demo_plot_info_var.set(src)
        except Exception:
            # Do not crash UI on plot issues
            pass
        ...
        try:
            if self.tiles and (tile_id == max(self.tiles.keys())):
                ...
                if "rx_text" in payload:
                    ...
                    self._demo_render_summary(rx_txt)
                    if export_info:
                        self._demo_update_embedded_waveform(export_info)
        except Exception:
            pass

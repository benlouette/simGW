...
class SimGwV2App:
	...
	def _demo_set_summary_kv(self, items) -> None:
	...
	def _demo_update_from_tile_payload(self, payload: dict, tile_id: int) -> None:
		try:
			st = payload.get('status')
			if st:
				self.demo_status_var.set(st)
			dev = payload.get('device_name') or payload.get('device') or ''
			addr = payload.get('device_addr') or ''
			if dev or addr:
				self.demo_device_var.set((dev + (' ' if dev and addr else '') + addr).strip())

			ei = None
			if hasattr(self, 'tile_export_info'):
				ei = self.tile_export_info.get(tile_id)
			if isinstance(ei, dict):
				count = int(ei.get('count', 0) or 0)
				if count > 0:
					self.demo_waveform_var.set(f'OK ({count * 64} points)')

			rx = payload.get('rx_text') or ''
			msg_type = ''
			m = re.match(r'\s*([a-zA-Z0-9_]+)\s*\{', rx)
			if m:
				msg_type = m.group(1)

			if msg_type == 'data_upload' and ('start_point' not in rx):
				pairs = rx.count('data_pair {')
				if pairs > 0:
					self.demo_overall_var.set(f'OK ({pairs} pairs)')

			if msg_type == 'data_upload' and ('start_point' in rx) and (self.demo_waveform_var.get() == '—'):
				tot_m = re.search(r'total_block:\s*(\d+)', rx)
				self.demo_waveform_var.set(f'Receiving ({tot_m.group(1)} blocks)' if tot_m else 'Receiving')

			items = []
			if msg_type:
				items.append(('Type', msg_type))
			if msg_type == 'current_version_upload':
				ver = re.search(r'version_string:\s*"([^"]+)"', rx)
				if ver:
					items.append(('Version', ver.group(1)))
			elif msg_type == 'config_hash_upload':
				h = re.search(r'config_hash:\s*(\d+)', rx)
				if h:
					items.append(('Config hash', h.group(1)))
			elif msg_type == 'data_upload':
				if 'start_point' in rx:
					sp = re.search(r'start_point:\s*(\d+)', rx)
					if sp:
						items.append(('Start', sp.group(1)))
					if isinstance(ei, dict) and int(ei.get('count', 0) or 0) > 0:
						items.append(('Points', str(int(ei.get('count')) * 64)))
				else:
					pairs = rx.count('data_pair {')
					if pairs > 0:
						items.append(('Pairs', str(pairs)))

			if items:
				self._demo_set_summary_kv(items)
		except Exception:
			return

	def _build_ui_demo(self, parent: tk.Frame) -> None:
		# Demo tab: minimal controls and at-a-glance info (no protocol changes).
		...
		ttk.Label(title_box, text='SimGW Demo', font=('Segoe UI', 14, 'bold')).pack(anchor='w')
		ttk.Label(title_box, text='At-a-glance session results', foreground='#666666').pack(anchor='w', pady=(2, 0))
		...
		info = ttk.LabelFrame(outer, text='Session')
		info.pack(fill=tk.X, pady=(0, 12))

		def kv_row(label: str, var: tk.StringVar) -> None:
			row = ttk.Frame(info)
			row.pack(fill=tk.X, padx=10, pady=4)
			ttk.Label(row, text=f'{label}:', width=14, foreground='#666666').pack(side=tk.LEFT)
			ttk.Label(row, textvariable=var).pack(side=tk.LEFT, fill=tk.X, expand=True)

		kv_row('Status', self.demo_status_var)
		kv_row('Device', self.demo_device_var)

		kpis = ttk.LabelFrame(outer, text='Results')
		kpis.pack(fill=tk.X, pady=(0, 12))

		row1 = ttk.Frame(kpis)
		row1.pack(fill=tk.X, padx=10, pady=6)
		ttk.Label(row1, text='Overall:', width=14, foreground='#666666').pack(side=tk.LEFT)
		ttk.Label(row1, textvariable=self.demo_overall_var).pack(side=tk.LEFT, fill=tk.X, expand=True)

		row2 = ttk.Frame(kpis)
		row2.pack(fill=tk.X, padx=10, pady=6)
		ttk.Label(row2, text='Waveform:', width=14, foreground='#666666').pack(side=tk.LEFT)
		ttk.Label(row2, textvariable=self.demo_waveform_var).pack(side=tk.LEFT, fill=tk.X, expand=True)

		summary = ttk.LabelFrame(outer, text='Latest message summary')
		summary.pack(fill=tk.BOTH, expand=True)

		self.demo_summary = tk.Text(summary, height=14, wrap='word', relief=tk.FLAT)
		self.demo_summary.pack(fill=tk.BOTH, expand=True, padx=10, pady=8)
		self.demo_summary.tag_configure('k', foreground='#666666', font=('Segoe UI', 10, 'bold'))
		self.demo_summary.tag_configure('v', foreground='#000000', font=('Segoe UI', 10))
		self.demo_summary.insert('1.0', 'No data yet. Click Start.')
		self.demo_summary.configure(state=tk.DISABLED)
	...
	def _apply_tile_update(self, tile_id: int, payload: Dict[str, str]) -> None:
		...
		try:
			if self.tiles and (tile_id == max(self.tiles.keys())):
				...
				# Update Demo view (friendly summary)
				self._demo_update_from_tile_payload(payload, tile_id)
		except Exception:
			pass

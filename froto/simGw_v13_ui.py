...
class BleCycleWorker:
    ...
    @staticmethod
    def _pretty_label_from_enum_token(token: str) -> str:
        if not token:
            return ""
        token = token.strip()
        s = token.replace("_", " ").strip().title()
        s = s.replace("Rms", "RMS").replace("Gps", "GPS")
        return s
        ...
        for pair in pairs:
            ...
            if not label:
                try:
                    ...
                    mm = re.search(r"\bmeasure_type\s*:\s*([A-Z0-9_]+)", ptxt)
                    if mm:
                        label = self._pretty_label_from_enum_token(mm.group(1))
                except Exception:
                    pass
            ...
            out.append({"label": label, "value": ", ".join(values) if values else "—"})
        return out
        ...
        try:
            ...
            if action == "connect_test":
                ...
                self.ui_queue.put(("tile_update", tile_id, {
                    ...
                    "overall_values": self._extract_overall_values(msg.data_upload) if msg_type == "data_upload" else None,
                }))
                ...
                self.ui_queue.put(("tile_update", tile_id, {
                    ...
                    "checklist": {"general_info_exchange": "done"},
                    "rx_text": f"TYPE: {msg_type}\nHEX: {self._hex_short(payload)}\n\n" + self._format_rx_payload(payload),
                    "overall_values": self._extract_overall_values(msg.data_upload) if (msg_type == "data_upload" and ("VIBRATION_ACC_WAVE" not in text_format.MessageToString(msg))) else None,
                }))
...
class SimGwV2App:
    def __init__(self, root: tk.Tk) -> None:
        ...
        self.demo_last_wave_export_info = None
    ...
    def _build_ui_demo(self, parent: tk.Frame) -> None:
        ...
        tk.Label(sum_in, text="Latest message summary", bg=self.colors["panel"], fg=self.colors["text"],
                 font=("Segoe UI", 11, "bold")).pack(anchor="w")

        self.demo_summary = tk.Text(sum_in, height=14, wrap=tk.WORD, bg=self.colors["panel"],
        ...
        # Embedded waveform plot (inside Demo tab)
        plot_box = tk.Frame(panel, bg=self.colors["panel"], highlightbackground=self.colors["border"], highlightthickness=1)
        ...
        tk.Label(header_row, text="Waveform (latest)", bg=self.colors["panel"], fg=self.colors["text"],
                 font=("Segoe UI", 11, "bold")).pack(side=tk.LEFT)
        self.demo_plot_label = tk.Label(header_row, text="(waiting...)", bg=self.colors["panel"], fg=self.colors["muted"])
        ...
        if Figure is None or FigureCanvasTkAgg is None:
            tk.Label(plot_area, text="Matplotlib/TkAgg not available (pip install matplotlib).",
                     bg=self.colors["panel"], fg=self.colors["muted"], justify=tk.LEFT).pack(anchor="w")
        else:
    ...
    def _demo_render_summary_combined(self) -> None:
        """Render Demo summary with two sections: Overall values + Waveform info."""
        if self.demo_summary is None:
            return

        overall_values = self.demo_last_overall_values or []
        wave_export = self.demo_last_wave_export_info or {}
        wave_txt = self.demo_last_wave_rx_text or ""

        total_block = None
        if wave_txt:
            mm = re.search(r"\btotal_block:\s*(\d+)", wave_txt)
            if mm:
                try:
                    total_block = int(mm.group(1))
                except Exception:
                    total_block = None
        ...
        if overall_values:
            ...
            for item in overall_values:
                lbl = str(item.get("label", "")).strip() or "Value"
                val = str(item.get("value", "")).strip() or "—"
        ...
        self.demo_summary.insert(tk.END, "\nWaveform\n", ("h",))
        if wave_export and isinstance(wave_export, dict) and wave_export.get("raw"):
            blocks = total_block if total_block is not None else (wave_export.get("count") or "?")
            pts = f"{int(blocks) * 64}" if isinstance(blocks, int) else "?"
    ...
    def _demo_plot_waveform_from_export(self, export_info: Optional[dict]) -> None:
        """Plot the waveform export into the embedded Demo canvas (raw bin)."""
        if self.demo_plot_canvas is None or self.demo_plot_fig is None:
            return
        if not export_info or not isinstance(export_info, dict):
            return
        raw_path = export_info.get("raw") or ""
        ...
        try:
            ...
            ax.set_ylabel("int16")
            ax.grid(True, alpha=0.2)
            self.demo_plot_canvas.draw_idle()
            if self.demo_plot_label is not None:
                self.demo_plot_label.configure(text=f"{meta.get('samples', len(y))} points")
        except Exception:
            return
    ...
    def _apply_tile_update(self, tile_id: int, payload: Dict[str, str]) -> None:
        ...
        if "overall_values" in payload and payload["overall_values"] is not None:
            self.latest_overall_values = payload["overall_values"]
        ...
        try:
            if self.tiles and (tile_id == max(self.tiles.keys())):
                ...
                # Demo KPIs and summary rendering
                export_info = payload.get("export_info") if payload.get("export_info") else None
                if "rx_text" in payload:
                    rx_txt = payload.get("rx_text", "")
                    self._demo_set_kpis_from_rx_text(rx_txt, export_info)
                    # Persist last overall + last waveform separately for Demo
                    try:
                        m = re.search(r"^TYPE:\s*([^\n]+)", (rx_txt or "").strip(), flags=re.MULTILINE)
                        msg_type = m.group(1).strip() if m else ""
                    except Exception:
                        msg_type = ""
                    if msg_type == "data_upload" and ("VIBRATION_ACC_WAVE" not in (rx_txt or "")):
                        self.demo_last_overall_rx_text = rx_txt
                        self.demo_last_overall_values = self.latest_overall_values
                    if msg_type == "data_upload" and ("VIBRATION_ACC_WAVE" in (rx_txt or "")):
                        self.demo_last_wave_rx_text = rx_txt
                        self.demo_last_wave_export_info = export_info
                        self._demo_plot_waveform_from_export(export_info)
                    self._demo_render_summary_combined()
        except Exception:
            pass

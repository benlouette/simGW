...
class BleCycleWorker:
    ...
    def _pretty_label_from_enum_token(self, token: str) -> str:
        ...
        a human-friendly label like 'Temperature'.
        """
        if not token:
            return "Value"
        t = token.strip()
        ...
        t = re.sub(r"(_CURRENT|_AVG|_MEAN|_RMS|_MIN|_MAX)$", "", t)
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    pass
                self.ui_queue.put(("tile_update", tile_id, {"status": latest_status, "rx_text": latest_rx_text, "export_info": export_info if "export_info" in locals() else None, "overall_values": overall_values if "overall_values" in locals() else None}))
...
class SimGwV2App:
    ...
    def _build_ui_demo(self, parent: tk.Frame) -> None:
        ...
        ttk.Button(btns, text="Clear", command=self._clear_tiles).pack(side=tk.LEFT)
        ...
        tk.Label(sum_in, text="Summary", bg=self.colors["panel"], fg=self.colors["text"], font=("Segoe UI", 11, "bold")).pack(anchor="w")
        ...
        self.demo_summary.pack(fill=tk.BOTH, expand=True, pady=(8, 10))
        ...
        tk.Label(header_row, text="Waveform", bg=self.colors["panel"], fg=self.colors["text"], font=("Segoe UI", 11, "bold")).pack(side=tk.LEFT)
    ...
    def _demo_plot_waveform_from_export(self, export_info: Optional[dict]) -> None:
        """Plot the latest waveform inside the Demo tab.

        Supports:
        - raw export (*.bin) via WaveformExportTools.extract_true_twf_samples_from_raw_export()
        - samples export (*_samples.csv) via _demo_plot_waveform_from_samples()
        """
        ...
        samples_path = ""
        try:
            raw_path = str(export_info.get("raw") or "")
            samples_path = str(export_info.get("samples") or "")
        except Exception:
            raw_path = ""
            samples_path = ""

        # Prefer raw export (works in your setup)
        if raw_path and os.path.isfile(raw_path):
            try:
                ...
                ax.set_title(f"Waveform (latest) • {meta.get('samples', len(y))} samples")
                ax.set_xlabel("Sample")
                ...
                # Fall back to samples.csv if available
                print(f"[DEMO] plot raw failed: {type(exc).__name__}: {exc}")
    ...
    def _demo_set_kpis_from_rx_text(self, rx_text: str, export_info: Optional[dict]) -> None:
        ...
        if msg_type == "data_upload" and "VIBRATION_ACC_WAVE" not in rx_text:
            ...
            if pairs > 0:
                ...
                metrics = self._demo_extract_key_metrics(rx_text)
                if metrics:
                    # Append key metrics if present (units depend on firmware definition)
                    tail = ", ".join(f"{k}={v}" for k, v in metrics.items())
                    self.demo_overall_var.set(f"OK ({pairs} pairs) • {tail}")
    ...
    def _apply_tile_update(self, tile_id: int, payload: Dict[str, str]) -> None:
        ...
        try:
            if self.tiles and (tile_id == max(self.tiles.keys())):
                ...
                if "checklist" in payload:
                    self._demo_update_timeline(payload.get("checklist") or {})
                ...
                if "rx_text" in payload:
                    ...
                    # Persist last overall + last waveform so the Demo summary can show both sections.
                    try:
                    ...
                    if msg_type == "data_upload" and "VIBRATION_ACC_WAVE" in (rx_txt or ""):
                        ...
                        try:
                            ...
                            print(f"[DEMO] plot update failed: {type(exc).__name__}: {exc}")

                    # If we received a waveform export but the RX text does not contain the expected token,
                    # still update the embedded plot (raw export is authoritative).
                    try:
                        if export_info and (export_info.get('raw') or export_info.get('samples')):
                            self._demo_plot_waveform_from_export(export_info)
                    except Exception as exc:
                        print(f"[DEMO] plot fallback failed: {type(exc).__name__}: {exc}")
